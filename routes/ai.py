...
def resume_gap():
    try:
        ...
        prompt = f"""
Reframe this career break positively for a resume:

Previous role: {prev_role}
Break duration: {break_duration}
Reason: {reason}

Write a professional, empowering 2-3 sentence statement highlighting transferable skills gained during the break.
Focus on growth, resilience, and readiness to return.
"""
...
def career_roadmap():
    try:
        ...
        prompt = f"""
Create a 4-step career roadmap for transitioning from {current_role} to {target_role} in {industry}.
